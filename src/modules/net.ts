...
class FakeSocket extends Duplex {
    ...
    override push(outgoingDataBuffer) {
        hooks.fromDriver(outgoingDataBuffer, this._parseMessage(outgoingDataBuffer));
    }

    sendMessageToDriver(requestId, message) {
        const bufferResponse = this._constructMessage(requestId, message)
        setTimeout(() => {
            this.stream._write(new Buffer(bufferResponse), null, () => null)
        }, 1);
    }

    _constructMessage(requestId, response) {
    ...
    // Just for logging
    _parseMessage(message: Uint8Array) {
